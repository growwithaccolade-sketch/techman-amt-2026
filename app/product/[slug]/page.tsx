      ...
      <main className="productPage shell">
        ...
        <section className="productHero">
          ...
          <div className="productDetail">
            ...
            <div className="detailPrice"><strong>{money(product.price)}</strong>{product.oldPrice && product.oldPrice > product.price && <del>{money(product.oldPrice)}</del>}</div>
            {product.oldPrice && product.oldPrice > product.price && <p className="savings">You save {money(product.oldPrice - product.price)}</p>}
            <div className="stockLine"><PackageCheck size={18}/><b>{product.stock > 0 ? "In stock" : "Out of stock"}</b><span>{product.stock > 0 ? `${product.stock} units available` : "Contact TechMan AMT for restock timing"}</span></div>
