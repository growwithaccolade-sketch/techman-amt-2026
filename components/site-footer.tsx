...
export default function SiteFooter() {
  ...
  return <footer className="footer premiumFooter">
    <div className="shell premiumFooterTop">
      ...
      <div><b>Support</b><a href={`tel:${settings.whatsappNumber}`}>Call {settings.whatsappNumber}</a><a href={`mailto:${settings.supportEmail}`}>{settings.supportEmail}</a>{supportLink && <a href={supportLink} target="_blank" rel="noreferrer">WhatsApp support</a>}<Link href="/faq">FAQs</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link></div>
    </div>
