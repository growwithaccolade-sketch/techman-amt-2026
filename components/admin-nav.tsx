...
import { BarChart3, Boxes, FileImage, FileText, LayoutDashboard, LogOut, Megaphone, MessageSquare, Package, Settings, ShoppingCart, Star, TicketPercent, Truck, UserCog, Users } from "lucide-react";
import { logoutAdmin } from "@/app/admin/actions";
import BrandLogo from "@/components/brand-logo";

export default function AdminNav({ active }: { active: "overview" | "products" | "orders" | "inventory" | "customers" | "analytics" | "settings" | "leads" | "promotions" | "reviews" | "delivery" | "marketing" | "staff" | "pages" | "media" }) {
  return (
    <aside className="adminSidebar">
      ...
      <nav>
        ...
        <Link className={active === "pages" ? "active" : ""} href="/admin/pages"><FileText size={18}/> Pages</Link><Link className={active === "media" ? "active" : ""} href="/admin/media"><FileImage size={18}/> Media</Link><Link className={active === "staff" ? "active" : ""} href="/admin/staff"><UserCog size={18}/> Staff</Link><Link className={active === "settings" ? "active" : ""} href="/admin/settings"><Settings size={18}/> Settings</Link>
      </nav>
